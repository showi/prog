...
load_data: function(res) {
	...
	this.set('node_parent_id', res.items[0].node_parent_id);
	...
	if (this.get('node_type') == 'people' && this.get('node_subtype') == 'entry') {
		var node = gshoNodesManager.get_node_by_id(res.items[0].node_id);;
	...
	this.set_loaded();
	gshoNodesManager.get_node_by_id(res.items[0].node_id).notify('sho_update');
	return 1;
},
