...
function ShoElement_tree_node_control(args) {
	...
	var txt = "";
	var node = gshoNodesManager.getNodeById(args.node_id);
...
function ShoTree_update(p_elm) {
	...
	node.children.each(function (n, index) { 
		...
		if ($(newChildID)) {
			...
			var nodeChild = gshoNodesManager.getNodeById(rootChild.getAttribute('node_id'));
			...
			var nodeChild = gshoNodesManager.getNodeById(n.node_id);
...
function ShoElement_tree_node(args) {
	...
	arrow.observe('sho_tree_node:click', function (event) {
		...
		gshoNodesManager.addNode({'dom_id': dom_id, 'node_id': node_id, 'elm': elm});
		if (statusElm) {
			var node_status = gshoNodesManager.getNodeById(node_id).get('node_status');
			if (node_status == 1){	
