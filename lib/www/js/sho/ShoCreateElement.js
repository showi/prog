...
function ShoTree_update(p_elm) {
	...
	var node_id = rootElm.getAttribute('sho_node_id');
	GSHO.log({type: 'info', msg: "ShoTree_update, root_id: " + rootElm.id + ", node_id: " + node_id} );
	if (!node_id) {
	...
	if (!is_defined(elmLabel)) {
		GSHO.log({type: 'error', msg: 'Cannot get elmLababek: '});
		return 0;
	}
	elmLabel.update(GSHO_NODES[node_id].node_name);
