...
function ShoCreateElement_node_control(args) {
	if (!args.dom_id) {
		GSHO.log({type: 'error', msg: 'ShoCreateElement_node_control, Cannot create node Element without dom_id'});
		return null;
	}
	if (!args.node_id) {
		GSHO.log({type: 'error', msg: 'ShoCreateElement_node_constrol, Cannot create node Element without node_id'});
		return null;
	}	
	...
	var status = new Element('img', {
		'alt'  : "node status",
		'href' : '#',
		'src'  : 'img/node_status_led_red.png',
...
//		GSHO.log("[sho_update] Node update, dom_id: " + dom_id + ", node_id: " + node_id);
	...
	arrow.observe('sho_tree_node:click', function (event) {
		...
		if (statusElm) {
			statusElm.src = "img/node_status_led_green.png";
		}
...
//			GSHO.log("Le node n'existe pas, on en crée un");
	...
	var nodeControl =  ShoCreateElement_node_control(args);
	/************ 
	 * ULCHILDS *
	 ************/
