...
var GSHO_URL_AJAX = "/app/php/feedNodi/";
...
function sho_object_shop_add(obj) {
...
function __callback_queue_event(time) {
	if (GSHO_QUEUE_EVENTS.length < 1) {
		setTimeout('__callback_queue_event('+time+')', 1000);
		return 0;
	}
	GSHO.log("deb num events: " + GSHO_QUEUE_EVENTS.length);
	var obj = GSHO_QUEUE_EVENTS[GSHO_QUEUE_EVENTS.length - 1];
	GSHO_QUEUE_EVENTS.pop();
	if (!obj) {
		return 0;
	}
	if (typeof(obj.obj[obj.name]) == "function") {
		GSHO.log("Dequeue event call " + obj.name + " on node_id:" + obj.obj.id);
		obj.obj[obj.name]();
	} else {
		GSHO.log("Invalid event '"+obj.name+"'");
	}
	GSHO.log("end num events: " + GSHO_QUEUE_EVENTS.length);
	setTimeout('__callback_queue_event('+time+')', time);
	return 0;
}

function __callback_watch_log(time) {
