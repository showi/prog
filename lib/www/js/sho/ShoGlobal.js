...
function __callback_queue_event(time) {
	var qlength = GSHO_QUEUE_EVENTS.length;
	if (qlength < 1) {
	...
	var i = 0;
	for(i = 0; i < qlength; i++) {
		var event = GSHO_QUEUE_EVENTS[i];
		if (!is_defined(event)) {
			continue;
		}
		if (event.name == 'load') {
			GSHO.log({type: 'info', msg: '__callback_queue_event, load event: processing...'});
			event.target[event.name]();
			continue;
		}
		newQueue.push(event);
	}
	GSHO_QUEUE_EVENTS = newQueue;
	qlength = GSHO_QUEUE_EVENTS.length;
	if (qlength < 1) {
		setTimeout('__callback_queue_event('+time+')', 1000);
		return 0;
	}
	var event = GSHO_QUEUE_EVENTS[qlength - 1];
	...
	if (typeof(event.target[event.name]) == "function") {
		...
		GSHO.log({type: 'error', msg: "Invalid event '"+event.name+"'" });
	}
