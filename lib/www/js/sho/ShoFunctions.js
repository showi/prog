...
//	if (!GSHO_XHR.wait()) {
//		var loop = 2;
//		while(!GSHO_XHR.wait() && (loop >= 0) && isDefined(event = ShoFunc_dequeue('load'))) {
//			loop--;
//			var myFunc = event.target[event.name];
//			if (isFunction(myFunc)) {
//				count++;
...
function __sholib_init() {
	...
	__callback_queue_event(100); // ms
	setTimeout('__callback_queue_event_xhr(1000)', 500);
}
